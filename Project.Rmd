---
title: "Final project, Student loan dept analysis"
date: "02/12/2019"
author: "Kelly Chen"
header-includes:
...
The data for this project comes from The Survey of Consumer Finances in 2016 ( which can be downloaded here https://www.federalreserve.gov/econres/scfindex.htm). The Survey of Consumer Finances(SFC) is provided by the Federal Reserve to help the government and ultimately the public to understand the financial condition of families in the United States. All the variables in the dataset are survey questions, the unit of observations is the US household. This dataset also comes with a codebook, which can also be downloaded on the same website.  In total, there are 31240 observations and 5320 variables. For the first part of the project, I chose 15 variables as predictors of the student loan amount. They are: Highest received education, Confidence in the US economy,  Knowledge about own finance, Loan amount, Willingness to take financial risk, How many loans, Owe money to purchase property, Trust fund, Main bank account balance, Income, Life insurance, Balance of saving account, Value of certificate of deposit, Value of saving bonds,  Market value of stock neutral funds. These are the financial indicators of a family. Basic demographic characteristics variables are also selected, including Age, Race, Gender of the head of the family,  Marital status and Kind of housing. The relationships between them and student loan amounts are graphed. For the second part of the project, I chose 6 variables to present a financial hardship. They are Loan amount, Credit denied, Late debt payment, Payday loan, Bankruptcy, Foreclosure. These variables are chosen based on the belief that they can well represent the financial hardship that a family faces.

When it comes to data wrangling, most of the work focused on interpreting the codebook. According to the codebook, I selected the above variables, rename all the columns as their original meanings. For categorical values, I also renamed them based on their meanings.  Some answers in the survey are not applicable or answered with “don’t know/not sure”, and they were given the value of 0 in the dataset. These values are treated as the missing value for the analysis and are turned into N/A during the data wrangling stage.

 
# Analysis and results

The first step for analysis is to find the relationship between the dependent variable (student loan amount) and independent variables.  For categorical variables that cannot be assigned to a certain level of loan amount relevance, I graphed their relationships to give a clear illustration of the difference of student loan amount between different groups.
...
# Discussion
One goal of this project is to predict the student loan amount based on the family attributes, which random forest does a good job on. Apart from the model prediction, different data visualizations also helped to see how student loan amount is different based on different family characteristics.  Another goal is to predict what type of financial hardship a family is likely to face based on their student loan amount. The random forest model does a good job on this prediction as well. 

Due to the limitation of this project, only a few variables were selected to predict student loans based on my assumption. More correlations can be revealed if all the variables are included. For the second part of the project, financial hardship can be defined as a qualitative outcome, predicted by the same variables chosen in part 2 using classification models. A new variable of financial hardship can then be generated and can be used to find if there is a linear relationship between the student loan amount and financial hardship. 
